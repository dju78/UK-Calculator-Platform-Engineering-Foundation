...
### B. Cloudflare Web Analytics (Visitor Traffic)
- **Purpose**: Privacy-first aggregate traffic analytics for the public website.
- **Metrics**: Total visitors, page views, top countries, top paths, referrers, device types.
- **Privacy Model**: No cookies, no IP logging, no persistent identifiers.
...
- **Purpose**: Official search performance metrics from Google Organic Search (`webmasters.readonly`).
...
| **Primary Units** | Unique Visitors & Total Page Views | Search Clicks & Search Impressions |
| **Query Breakdown** | Not applicable (Referrer source only) | Keyword query strings and SERP ranking positions |
| **Privacy Scope** | Aggregate page counts without IP storage | Aggregated anonymized search terms |