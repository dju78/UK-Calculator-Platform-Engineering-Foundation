  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 border-b border-slate-200 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Single-pane operational visibility and governance monitoring for the UK Calculator Platform.
            </p>
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <MetricCard
              ...
              value={rulesOverview.lastChecked}
              subtext="Primary sources schedule audited"
              source="statutory calendar"
            />
          </div>
        </div>

        {/* Section 3: QA & Verification */}
        <div>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <h2 className="text-xs font-bold uppercase tracking-wider text-slate-700">
        ...
        {/* Section 4: Search & SEO Readiness */}
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-xs font-bold uppercase tracking-wider text-slate-700">
              4. Search & SEO Readiness
            </h2>
            <Link href="/seo" className="text-xs font-semibold text-slate-900 hover:text-blue-600 hover:underline">
              SEO & IndexNow Details →
            </Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <MetricCard
              label="Canonical Production Host"
              value="ukcalc.jomovate.com"
              subtext="Unified canonical URL across all routes"
              source="site.ts"
            />
            <MetricCard
        ...
        {/* Section 5: Releases & System Architecture */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-xs">
            ...
            <div className="space-y-2.5 text-xs">
              {releaseOverview.milestones.slice(0, 4).map((m) => (
                <div key={m.codename} className="flex items-center justify-between p-2 rounded bg-slate-50 border border-slate-100">
                  <div>
                    <div className="font-semibold text-slate-900">{m.codename}</div>
                    <div className="text-slate-500 text-[11px]">{m.description}</div>
                  </div>
                  <div className="text-right">
                    <StatusBadge status={m.status} />
                    <div className="text-[10px] text-slate-400 font-mono mt-1">{m.date}</div>
                  </div>
                </div>
              ))}
            </div>
