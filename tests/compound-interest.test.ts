...
for (const fixture of wave1Benchmarks["INV-002"]) {
  test(`INV-002 benchmark: ${fixture.scenario}`, () => {
    const result = calculate("INV-002", fixture.inputs, { now: new Date("2026-08-22T08:00:00Z") });
...
test("INV-002 returns required reproducibility metadata", () => {
  const result = calculate("INV-002", {P:10000, nominal_rate:0.05, m:12, years:10}, {now:new Date("2026-08-22T08:00:00Z")});
...
test("INV-002 rejects invalid compounding frequency", () => {
  assert.throws(() => calculate("INV-002", {P:1000, nominal_rate:0.05, m:0, years:10}), CalculationValidationError);
});
